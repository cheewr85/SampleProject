...
- 설정한 타이머에 맞춰서 카운트 다운을 하면서 화면을 갱신함

- 시간이 흐를 때 소리와 시간이 끝났을 때 알람음이 울림

## 메인 화면
- SeekBar를 통해서 시간을 조절하고 해당 시간이 반영이 됨

- 배경색을 직접 Themes에서 사전에 설정한 color를 가지고 설정하고 SeekBar의 경우도 ProgressDrawable을 투명으로 그리고 SeekBar를 눌러서 조절하는 바 역시 Vector Asset을 활용함

- 이미지 역시 뽀모도로를 나타내기 위해서 외부 파일을 다운받아 ImageView에서 적용시킴
### 실행 화면
![one](/Basic/Pomodoro/img/one.png)
![one](/Basic/Pomodoro/img/two.png)

- Drawable 폴더에 직접 추가를 함, Vector Asset으로 추가하고 Drawable을 직접 그림
...
### 구현 코드
- SeekBar의 경우 상태에 따라 상황이 바뀌므로 해당 인터페이스를 가지고 와 상황에 맞게 설정함

- SeekBar을 건드렸을 때 상황에 따라 카운트다운을 시작하고 실행시키고 정지시킴

- SoundPool을 활용하여서 시간이 흐를 때 나는 소리와 완료됐을 때 나는 소리를 load 하여 상황에 맞게 나도록 설정함

- 소리를 내는 것 자체가 앱 자체적인것이 아닌 디바이스를 매개로 소리가 나는 것이므로 생명주기를 고려해서 소리를 조절함, 그렇게 하지 않으면 앱을 백그라운드로 넘겨도 소리가 남

- 중복되는 구현 요소들에 대해서는 추상화로 메소드화 하였음
